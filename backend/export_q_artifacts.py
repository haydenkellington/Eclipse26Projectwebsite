...
def _state_key_from_augmented_state(state: tuple[Any, ...], outs: int = 0) -> str:
...
def export_augmented_q_table(
    ...
    The current notebook state does not include outs, while the website/API does.
    By default, each notebook state is copied across outs 0, 1, and 2 so the
    dashboard can use the table until outs are added to notebook training.
    """
    ...
    for key, q_value in agent.Q.items():
        ...
        if int(state[-1]) != -1:
            state = (*state[:-1], idx_to_pitch[int(state[-1])])

        for outs in outs_values:
            state_key = _state_key_from_augmented_state(state, outs=outs)
